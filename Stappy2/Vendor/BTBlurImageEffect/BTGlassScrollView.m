...
@implementation BTGlassScrollView
{
    ...
    RandomImageView *_topMaskImageView;
}

-(void)setNeedsBlur:(BOOL)needsBlur {
    self.backgroundImageView.needsBlur = needsBlur;
    self.blurredBackgroundImageView.needsBlur = needsBlur;
    _needsBlur = needsBlur;
}

-(void)setRemoveRandomImages:(BOOL)removeRandomImages {
    if (!removeRandomImages) {
        self.backgroundImageView.needsBlur = self.needsBlur;
        self.blurredBackgroundImageView.needsBlur = self.needsBlur;
    } else {
        self.backgroundImageView.image = self.backgroundImage;
        self.blurredBackgroundImageView.image = self.blurredBackgroundImage;
    }

    _removeRandomImages = removeRandomImages;
}
...
- (void)setFrame:(CGRect)frame
{
    ...
    [_foregroundView setFrame:CGRectOffset(_foregroundView.bounds, roundf((_foregroundScrollView.frame.size.width - _foregroundView.bounds.size.width)/2), _foregroundScrollView.frame.size.height - _foregroundScrollView.contentInset.top - _viewDistanceFromBottom)];
...
- (void)setTopLayoutGuideLength:(CGFloat)topLayoutGuideLength
{
    ...
    //reposition
    [_foregroundView setFrame:CGRectOffset(_foregroundView.bounds, roundf((_foregroundScrollView.frame.size.width - _foregroundView.bounds.size.width)/2), roundf(_foregroundScrollView.frame.size.height - _foregroundScrollView.contentInset.top - _viewDistanceFromBottom))];
...
- (void)setBackgroundImage:(UIImage *)backgroundImage overWriteBlur:(BOOL)overWriteBlur animated:(BOOL)animated duration:(NSTimeInterval)interval
{
    ...
    if (animated) {
        RandomImageView *previousBackgroundImageView = _backgroundImageView;
        previousBackgroundImageView.needsBlur = NO;
        RandomImageView *previousBlurredBackgroundImageView = _blurredBackgroundImageView;
        previousBackgroundImageView.needsBlur = YES;
...
- (void)createBackgroundImageView
{
    ...
    _backgroundImageView = [[RandomImageView alloc] init];
    _backgroundImageView.needsBlur = NO;
//    _backgroundImageView = [[UIImageView alloc] initWithImage:_backgroundImage];
    ...
    _blurredBackgroundImageView = [[RandomImageView alloc] init];
    _blurredBackgroundImageView.needsBlur = YES;
//    _blurredBackgroundImageView = [[RandomImageView alloc] initWithImage:_blurredBackgroundImage];
...
- (void)createForegroundView
{
    ...
    [_foregroundView setFrame:CGRectOffset(_foregroundView.bounds, roundf((_foregroundScrollView.frame.size.width - _foregroundView.bounds.size.width)/2), _foregroundScrollView.frame.size.height - _viewDistanceFromBottom)];
...
- (void)scrollViewDidScroll:(UIScrollView *)scrollView
{
    ...
    //pass to delegate ths scroll event
